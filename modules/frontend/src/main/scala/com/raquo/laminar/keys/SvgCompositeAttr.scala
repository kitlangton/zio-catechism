...
import com.raquo.laminar.api.Laminar.{ SvgElement, MapValueMapper, StringValueMapper }
...
class SvgCompositeAttr[Attr <: Key](val key: Attr, separator: Char) {

  // @TODO[API] Should StringValueMapper be passed implicitly?
  @inline def apply(items: String): Setter[SvgElement] = {
    update(StringValueMapper.toDict(items, separator))
  }

  @inline def apply(items: Map[String, Boolean]): Setter[SvgElement] = {
    update(MapValueMapper.toDict(items, separator))
  }

  @inline def apply[V](items: V*)(implicit mapper: CompositeValueMapper[collection.Seq[V]]): Setter[SvgElement] = {
    update(mapper.toDict(items, separator))
  }

  @inline def :=(items: String): Setter[SvgElement] = {
    update(StringValueMapper.toDict(items, separator))
  }

  @inline def :=(items: Map[String, Boolean]): Setter[SvgElement] = {
    update(MapValueMapper.toDict(items, separator))
  }

  @inline def :=[V](value: V*)(implicit mapper: CompositeValueMapper[collection.Seq[V]]): Setter[SvgElement] = {
  ...
  def set(newItems: String): Setter[SvgElement] = {
    Setter { element =>
      element.ref.setAttributeNS(namespaceURI = null, qualifiedName = key.name, newItems)
    }
  }

  def <--[V]($items: Observable[V])(implicit valueMapper: CompositeValueMapper[V]): Binder[SvgElement] = {
  ...
  private def update(newItems: js.Dictionary[Boolean]): Setter[SvgElement] = {
    Setter { element =>
